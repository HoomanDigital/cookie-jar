  ...
  try {
    if (window.ethereum) {
      ...
      provider = new ethers.providers.JsonRpcProvider(
        `https://eth-mainnet.g.alchemy.com/v2/${rpcUrl}`
      );
    }
...
const useEnsName = (provider: ethers.providers.Provider, address: string) => {
  return useQuery({
    queryKey: ["ensName", address],
    queryFn: () => provider.lookupAddress(address),
    staleTime: 60 * 60 * 1000,
  });
};

const useNftImage = (tokenId: string, nftContractAddress: string) => {
  return useQuery({
    queryKey: ["nftImage", tokenId, nftContractAddress],
    queryFn: async () => {
      if (tokenId === "0") return null;

      const response = await fetch(
        `https://api.opensea.io/api/v2/chain/ethereum/contract/${nftContractAddress}/nfts/${tokenId}`,
        {
          headers: {
      ...
      return nftData.nft?.image_url || nftData.image_url;
    },
    staleTime: 60 * 60 * 1000,
    enabled: tokenId !== "0",
  });
...
function WithdrawalHistoryItem({
  ...
  const { data: ensName } = useEnsName(provider, event.user);
  const { data: nftImageUrl } = useNftImage(event.tokenId, nftContractAddress);
  ...
  return (
    ...
    >
      <div className="p-5 bg-slate-50/50">
        ...
        {event.tokenId !== "0" && nftImageUrl && (
          <div className="mb-4 border-t border-b border-slate-200 py-4">
            <div className="flex items-center gap-2 mb-3 text-slate-700">
              <ImageIcon className="h-5 w-5 text-indigo-500" />
              <span className="font-semibold text-sm">MoonShotBots</span>
              <span
                className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider
                ${
                  event.tokenId !== "0"
              ...
              >
                {event.tokenId !== "0" ? `#${event.tokenId}` : "Whitelist"}
              </span>
            </div>
            <div className="flex justify-center">
              <a
                href={`https://opensea.io/assets/ethereum/${nftContractAddress}/${event.tokenId}`}
                target="_blank"
              ...
              >
                <img
                  src={nftImageUrl}
                  alt={`NFT #${event.tokenId}`}
                  className="w-full h-auto object-cover"
                  onError={(e) => {
                    const target = e.target as HTMLImageElement;
                    target.src =
                      "https://via.placeholder.com/280x280?text=NFT+Image+Not+Found";
                  }}
                />
              </a>
            </div>
...
export function WithdrawalHistory({
  ...
  nftContractAddress = "0x8b13e88EAd7EF8075b58c94a7EB18A89FD729B18",
}: {
  contractAddress: string;
  rpcUrl?: string;
  nftContractAddress?: string;
